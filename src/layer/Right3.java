...
public class Right3 extends Layer {

	public Right3(Level level) {
		super(level);
		plates = new ArrayList<>();
		logger.debug("Right3 created successfully");
	}
	...
	public void move(){
		for(int i = 0 ; i < plates.size() ; i++) {
			...
			if(plate.getState() == 0) {
				if(plate.getCenter().x <= cons.getWidth() - cons.getWidth() / 10) {
					changeState(plate);
				}
				else{
	...
	public void addPlate() {
		temp = pool.getPlate();
		temp.setCenter(1150, 95);
		plates.add(temp);
		logger.debug("a plate is added");
	}
