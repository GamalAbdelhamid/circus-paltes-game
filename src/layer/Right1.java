...
public class Right1 extends Layer {

	public Right1(Level level) {
		super(level);
		plates = new ArrayList<>();
		logger.debug("Right1 created successfully");
	...
	public void move(){
		for(int i = 0 ; i < plates.size() ; i++) {
			...
			if(plate.getState() == 0) {
				if(plate.getCenter().x <= cons.getWidth() - cons.getWidth() / 3) {
					changeState(plate);
				}
				else{
